...
export default function StoryReport({ stats, quality, years, year, isDemo, onYear, onDetail, onImport, onShare }) {
  ...
  const topMonthIndex = stats.monthCounts.indexOf(Math.max(...stats.monthCounts));
  ...
  return <div className="story-report">
    ...
    <main className="story-stage" ref={scrollerRef}>
      ...
      <StorySlide index={2} eyebrow="DAYS & NIGHTS" tone="ink">
        {messageReady ? <><p className="story-lead">你在 <b>{stats.activeDays}</b> 天里说过话</p><h2>最长连续出现<br/><strong>{stats.streak} 天</strong></h2><div className="story-facts"><span><small>最常出现</small><b>{String(stats.peakHour).padStart(2, "0")}:00</b></span><span><small>最热闹的月份</small><b>{topMonthIndex + 1} 月</b></span><span><small>那个月发出</small><b>{number.format(stats.monthCounts[topMonthIndex] || 0)} 条</b></span></div></> : <MissingBlock title="还不知道你的聊天节奏" text="不读取聊天语义，只从时间戳计算活跃日、连续天数和高峰时段。" onImport={onImport} inverted />}
      </StorySlide>

      <StorySlide index={3} eyebrow="HOW YOU EXPRESSED YOURSELF">
        {messageReady ? <><p className="story-lead">你最常用</p><h2 className="story-word">{typeLabels[topType?.[0]] || "文字"}</h2><p className="story-note">共 {number.format(topType?.[1] || 0)} 条。图片 {number.format(stats.typeCounts.image || 0)} 张，语音约 {number.format(stats.voiceMinutes)} 分钟，表情 {number.format(stats.typeCounts.sticker || 0)} 个。</p></> : <MissingBlock title="表达方式等待导入" text="支持文字、图片、语音、视频、表情、链接、文件和拍一拍等类型。" onImport={onImport} />}
      </StorySlide>

...
当前版本：**V0.2.1 · 全记录导入与安全修复版**

微信年轮是一款本地优先的微信年度报告工具。把自己的聊天 JSON/CSV、微信支付个人对账 CSV、朋友圈结构化记录或微信年轮标准 JSON 一次多选（也可选择整个导出文件夹）后，它会在本机合并去重并统计消息、活跃日、可检测撤回、红包、转账和朋友圈足迹，再生成简洁的故事式年报与隐私分享图。
...
公开 GitHub 仓库推送新的 `v*` 标签后，GitHub Actions 会测试、审计并免费构建：

- macOS Apple 芯片（arm64）DMG + ZIP
- macOS Intel（x64）DMG + ZIP
- Windows x64 安装版 + 便携版

每个 Release 还会附带 `SHA256SUMS.txt` 与 GitHub 构建来源证明。安装包暂未付费签名，因此系统会显示“未知开发者/未知发布者”；这与源码和构建可验证性是两件事，核对与打开方法写在 [发布教程](docs/BEGINNER-GITHUB-GUIDE.md) 中。

## 导入格式

1. 微信支付：官方“用于个人对账”的 CSV；只有账单声明范围覆盖完整年份时才标为完整。
2. 聊天：常见 JSON/CSV，以及多种 EchoTrace/WeFlow 风格字段和嵌套会话结构。
3. 朋友圈：包含 `time`、`type`、`isOwn` 等字段的 JSON/CSV；没有本人归属字段的记录不会擅自计入。
4. 微信年轮标准 JSON：可在“数据与隐私”页按当前年份或全部年份导出后再次导入。
5. 可同时多选以上文件，桌面端也可选择整个文件夹；图片、视频和微信原始数据库不会被当作结构化导入文件。
...
- 解析在隔离的本地后台线程完成；Electron 拒绝任意本地导航、网页嵌入和未使用的系统权限。
...
项目自身代码采用 [MIT License](LICENSE)。第三方项目仍遵循各自许可证；微信年轮 V0.2.1 未复制或捆绑 EchoTrace、WeFlow、CipherTalk 的取密钥或解密实现。
